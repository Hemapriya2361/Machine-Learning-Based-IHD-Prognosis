  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
      ...
      <div className="card" style={{ gridColumn: "1/-1" }}>
        <div className="card-title"> Kaplan-Meier Survival Curves — RSF Risk Stratification</div>
        ...
        <ResponsiveContainer width="100%" height={280}>
          <LineChart>
            ...
            <XAxis
              dataKey="t" type="number" domain={[0, 8]}
              label={{ value: "Time (Years)", position: "bottom", fill: COLORS.textMuted, fontSize: 11 }}
            ...
            <YAxis
              ...
              tick={{ fill: COLORS.textMuted, fontSize: 10 }}
              label={{ value: "Survival Prob.", angle: -90, position: "left", fill: COLORS.textMuted, fontSize: 11 }}
            />
            <Tooltip
              formatter={(v, n) => [(v * 100).toFixed(1) + "%", n]}
              contentStyle={tooltipStyle}
            />
            <ReferenceLine x={7} stroke={COLORS.borderStrong} strokeDasharray="4 4"
              label={{ value: "7yr horizon", fill: COLORS.textMuted, fontSize: 10 }}
            />
        ...
        <div style={{ marginTop: 14, borderTop: "1px solid #E8E4DC", paddingTop: 12 }}>
          <div style={{ fontSize: 10, color: COLORS.textMuted, marginBottom: 6, letterSpacing: 0.5, textTransform: "uppercase", fontWeight: 600 }}>
            Number at risk
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "80px repeat(6, 1fr)", fontSize: 11 }}>
            {["", "t=0", "t=2", "t=4", "t=5", "t=6", "t=8"].map((h, i) => (
              <div key={i} style={{ padding: "3px 6px", color: COLORS.textMuted, fontWeight: i === 0 ? 700 : 400 }}>{h}</div>
            ))}
            {AT_RISK.map(([label, color, vals]) => [
              <div key={label} style={{ padding: "3px 6px", color, fontWeight: 700, fontSize: 10 }}>{label}</div>,
              ...vals.map((v, i) => (
                <div key={label + i} style={{ padding: "3px 6px", color, fontSize: 11, fontFamily: "'DM Mono', monospace" }}>{v}</div>
              )),
            ])}
          </div>
        </div>
      ...
      {/* Individual survival S(t) */}
      <div className="card">
        <div className="card-title"> Individual Survival Curve — Current Patient</div>
        <div style={{ fontSize: 11, color: COLORS.textMuted, marginBottom: 14 }}>
          RSF predicted S(t) · 7yr risk ={" "}
          <span style={{ color: riskColor, fontWeight: 700 }}>{result.risk_7yr}%</span>
        </div>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={survCurve}>
            <CartesianGrid strokeDasharray="3 3" stroke="#E8E4DC" />
            <XAxis dataKey="t" tickFormatter={(v) => v.toFixed(0)} label={{ value: "Years", position: "bottom", fill: COLORS.textMuted, fontSize: 10 }} tick={{ fill: COLORS.textMuted, fontSize: 10 }} />
            <YAxis domain={[0, 1]} tickFormatter={(v) => (v * 100).toFixed(0) + "%"} tick={{ fill: COLORS.textMuted, fontSize: 10 }} />
            <ReferenceLine x={7} stroke={COLORS.borderStrong} strokeDasharray="4 4" />
            <Tooltip formatter={(v) => [(v * 100).toFixed(1) + "%", "S(t)"]} contentStyle={tooltipStyle} />
      ...
      <div className="card">
        <div className="card-title">Cumulative Hazard H(t) — Current Patient</div>
        <div style={{ fontSize: 11, color: COLORS.textMuted, marginBottom: 14 }}>RSF predicted cumulative hazard function</div>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={hazardCurve}>
            ...
            <ReferenceLine x={7} stroke={COLORS.borderStrong} strokeDasharray="4 4" />
            <Tooltip formatter={(v) => [v.toFixed(4), "H(t)"]} contentStyle={tooltipStyle} />
      ...
      {/* Calibration summary */}
      <div className="card" style={{ gridColumn: "1/-1" }}>
        <div className="card-title">Model Calibration Summary</div>
          ...
          {CALIBRATION_ROWS.map(([label, val, note, col]) => (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                ...
                <span style={{ fontWeight: 800, color: col, fontFamily: "'DM Mono', monospace" }}>{val}</span>
              </div>
